init python:
    ...
    class columns_glitch(renpy.Displayable):
        """
        `squareside`
            The size, in pixels, of the side of the squares the child image will be cut to. This will
            be adjusted so that all the "squares" (rectangles, really) have the same width and the
        ...
        @staticmethod
        def glitch(child, cwidth, cheight, randomobj, squareside=20, chroma=.25):
            if not renpy.display.render.models:
                chroma = False
            if not (cwidth and cheight):
                return child
            ...
            for y in range(nrows):
                for x in range(ncols):
                    height_mod = randomobj.randrange(10, 100)
                    lizt.append(Transform(child,
                                            crop=(absolute(x*square_width), absolute((y*square_height)-height_mod), square_width, square_height-height_mod),
                                            subpixel=True,
                                            ))
